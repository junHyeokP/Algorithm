...
public class QuickSort extends Sort {
	...
	@Override
	public void sort(int[] dataList) { // 추상 부모 메서드 오버라이딩
	...
	void quickSort(int[] sortedData, int start, int end) { // 빠른 정렬, 정렬된 데이터와 시작점, 끝을 매개변수로 가져옴
	...
	int partition(int[] dataList, int start, int end) { // 파티션 (분할)
		int pivot = dataList[end]; 						// 배열의 끝부분을 가리키는 변수를 pivot으로 지정 (pivot은 빠른 정렬에서 "배열의 분할"에 공통적으로 사용되는 변수의 이름이다, 주로 배열에 있는 요소들 중 하나로 pivot의 위치를 정하지만, 여기선 배열의 끝을 지정하는 기법을 사용했다.) 
		int leftEnd = start - 1;						// 분열한 배열의 시작 = 배열의 시작 - 1
		for (int i = start; i <= end - 1; i++) { 		// for문, 변수 i로 배열의 시작부터 끝까지 탐색하도록 설정
			if (dataList[i] < pivot) { 					// i번째 인덱스의 데이터가 pivot보다 작을경우
