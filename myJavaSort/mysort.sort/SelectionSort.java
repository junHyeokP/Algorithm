...
public class SelectionSort extends Sort { // 선택 정렬, 정렬 안된 부분에서 최대 값을 찾아서 정렬 안된 부분의 가장 오른쪽 혹은 왼쪽 데이터와 교환하는 알고리즘
	...
	int selectMax(int[] dataList, int last) { // 최대 값의 인덱스를 찾아 반환하는 메서드
		int max = dataList[0]; // 최대 값을 넣을 변수
